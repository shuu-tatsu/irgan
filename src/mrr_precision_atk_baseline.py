...
def precision_at(i, batch_scores, testList, top_k):
    score_index_list = []
    for index, s in enumerate(batch_scores):
        score_index_list.append([index, s])	

    score_index_sorted_list = sorted(score_index_list, key=itemgetter(1), reverse=True)
    sorted_k_list = score_index_sorted_list[:top_k]
    index_list = [index for index, s in sorted_k_list]

    score_k_list = []
    for index in index_list:
        if int(testList[i*500+index].split()[0])==1:
            #予測した候補のラベルが１ならscoreを加算
            score_k_list.append(1)
        else:
            score_k_list.append(0)

    #制約緩和
    #score = sum(score_k_list) * 1.0 / top_k
    score = sum(score_k_list) * 1.0
    return score


def rr_for_mrr(i, batch_scores, testList):
    score_index_list = []
    for index, s in enumerate(batch_scores):
        score_index_list.append([index, s])	

    score_index_sorted_list = sorted(score_index_list, key=itemgetter(1), reverse=True)
    index_list = [index for index, s in score_index_sorted_list]

    ranking = 500
    for rr_index, index in enumerate(index_list):
        if int(testList[i*500+index].split()[0])==1:
            #予測した候補のラベルが１
            ranking = rr_index + 1
            break
...
def evaluation(sess,model,log,num_epochs=0):
    ...
    mrr_current, precision_current=dev_step(sess,model,test1List,1800)
    line="test1: %d epoch: mrr %f precision %f"%(current_step, mrr_current, precision_current)
    print (line)
    with open('./result/mrr_precision_at_k.log', 'a') as a:
        a.write(line)
        a.write('\n')
    return 0
...
def load_model_list():
    ...
    epoch = 0
    d_list = []
    with open(file_path_read, 'r') as r:
        lines = r.readlines()
        for line in lines:
            ...
            for model in line_split:
                if 'Dis' in model:
                    ...
                    d_list.append([val, time, name])
        d_list.sort(key=itemgetter(1))
        name_list = [name for val, time, name in d_list]
        return name_list
...
if __name__ == '__main__':
    load_list = load_model_list()
    name_list = []
    for model_name in load_list:
        name_list.append(model_name)

    for dis_model in name_list:
        DIS_MODEL_FILE="model/" + dis_model
