...
class QACNN():
    ...
    def __init__(self, sequence_length, batch_size,vocab_size,
        ...
        with tf.name_scope("embedding"):
            if self.paras==None:
                if self.embeddings ==None:
                    ...
                    self.Embedding_W = tf.Variable(np.array(self.embeddings),name="embedding_W",
        ...
        with tf.name_scope('conv-maxpool-all') as scope:
    ...
    def getRepresentation(self,sentence):
        ...
        pooled_reshape = tf.reshape(tf.concat(output, 3), [-1, self.num_filters_total]) #add 
