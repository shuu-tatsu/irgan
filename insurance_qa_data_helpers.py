...
def build_vocab():
    ...
    for word, count in counter.most_common():
        ...
        if count >= 15:
            vocab[word] = cnt
            cnt += 1
    vocab[u'UNKNOWN'] = len(vocab)
    return vocab


def read_alist():
    alist = []
    for line in open('chieQA/train'):
        items = line.strip().split(' ')
...
def load_vectors(vocab=None):
    if vocab==None:
        return
    vectors = {}
    for line in open('chieQA/vectors.nobin'):
        items = line.strip().split(' ')
        if (len(items) < 101):
            continue
        vec = []
...
def loadTestSet(filename):
    testList = []
    for line in open('chieQA/'+filename):
