...
#tf.flags.DEFINE_integer("num_epochs", 2, "Number of training epochs (default: 200)") #toy
...
#tf.flags.DEFINE_integer("pools_size", 10, "The sampled set of a positive ample, which is bigger than 500") #toy
...
def generate_gan(sess, model,loss_type="pair",negative_size=3):
    ...
    for _index ,pair in enumerate (raw):
        ...
        for batch in insurance_qa_data_helpers.batch_iter(canditates,batch_size=FLAGS.batch_size):                          
            ...
            predicted=sess.run(model.gan_score,feed_dict)  #gan_scoreは cos(q,neg) - cos(q,pos)
    ...
    for i in range(dev_size):
        ...
        for j in range(int(500/FLAGS.batch_size)):
            x_test_1, x_test_2, x_test_3 = insurance_qa_data_helpers.load_val_batch(
                                           testList, vocab, i*500+j*FLAGS.batch_size, FLAGS.batch_size)
            feed_dict = {
            ...
            with sess.as_default(), open(log_precision,"w") as log, open(loss_precision,"w") as loss_log:
                ...
                with tf.name_scope('gan_training') as scope:
                    for i in range(FLAGS.num_epochs):
                        with tf.name_scope('discriminator_gan_training') as scope:
                            if i>0:
                                samples=generate_gan(sess,generator) 
                                # for j in range(FLAGS.d_epochs_num):                           
                                for _index,batch in enumerate(insurance_qa_data_helpers.batch_iter(samples,
                                    ...
                                    line=("%s: DIS step %d, loss %f with acc %f "%(datetime.datetime.now().isoformat(),
                                                                                   step,
                                                                                   current_loss,
                                                                                   accuracy))
                                    if _index%10==0:
                        ...
                        with tf.name_scope('generator_gan_training') as scope:
                            for g_epoch in range(FLAGS.g_epochs_num):   
                                for _index, pair in enumerate(raw):
                                    ...
                                    predicteds=[]
                                    for batch in insurance_qa_data_helpers.batch_iter(samples,
                                                                                      batch_size=FLAGS.batch_size):
                                    ...
                                    _, step, current_loss, positive,negative = sess.run([generator.gan_updates,
                                    ...
                                    #tw = tf.summary.FileWriter("log_dir", graph=sess.graph) #add
                                    line=("%s: GEN step %d, loss %f  positive %f negative %f"%(datetime.datetime.now().isoformat(),
                                                                                               step,
